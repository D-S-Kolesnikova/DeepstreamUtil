    ...
    if (col < width && row < height)
    {
        for (unsigned int k = 0; k < 3; k++)
        {
            outBuffer[width * height * k + row * width + col] =
                scaleFactor[k] * inBuffer[row * pitch + col * inputPixelSize + k];
        }
    }
}
    ...
    float *meanDataBuffer)
{
    ...
    if (col < width && row < height)
    {
        ...
        {
            outBuffer[width * height * k + row * width + col] =
                scaleFactor[(row * width * 3) + (col * 3) + k] * ((float) inBuffer[row * pitch + col * inputPixelSize + k] -
    ...
    float *meanDataBuffer)
{
    ...
    if (col < width && row < height)
    {
        ...
        {
            outBuffer[row * width * 3 + col * 3 + k] =
                scaleFactor[k] * ((float) inBuffer[row * pitch + col * inputPixelSize + k] -
                meanDataBuffer[(row * width * 3) + (col * 3) + k]);
        }
    }
}
    ...
    float *scaleFactor)
{
    ...
    if (col < width && row < height)
    {
        ...
        {
            outBuffer[width * height * k + row * width + col] =
                scaleFactor[(row * width * 3) + (col * 3) + k] * inBuffer[row * pitch + col * inputPixelSize + (2 - k)];
        }
    }
}
    ...
    float *scaleFactor)
{
    ...
    if (col < width && row < height)
    {
        ...
        {
            outBuffer[row * width * 3 + col * 3 + k] =
                scaleFactor[k] * inBuffer[row * pitch + col * inputPixelSize + (2 - k)];
        }
    }
}
    ...
    float *meanDataBuffer)
{
    ...
    if (col < width && row < height)
    {
        ...
        {
            outBuffer[width * height * k + row * width + col] =
                scaleFactor[(row * width * 3) + (col * 3) + k] * ((float) inBuffer[row * pitch + col * inputPixelSize + (2 - k)] -
                meanDataBuffer[(row * width * 3) + (col * 3) + k]);
        }
    }
}
    ...
    float *meanDataBuffer)
{
    ...
    if (col < width && row < height)
    {
        ...
        {
            outBuffer[row * width * 3 + col * 3 + k] =
                scaleFactor[k] * ((float) inBuffer[row * pitch + col * inputPixelSize + (2 - k)] -
                meanDataBuffer[(row * width * 3) + (col * 3) + k]);
        }
    }
}
        ...
        float *scaleFactor)
{
    ...
    if (col < width && row < height)
    {
        outBuffer[row * width + col] = scaleFactor[0] * inBuffer[row * pitch + col];
    }
}
        ...
        float *meanDataBuffer)
{
    ...
    if (col < width && row < height)
    {
        outBuffer[row * width + col] =
            scaleFactor[0] * ((float) inBuffer[row * pitch + col] -
        ...
        float *scaleFactor)
{
    ...
    if (col < width && row < height)
    {
        outBuffer[row * width + col] = scaleFactor[0] * inBuffer[row * width + col];
    }
}
        ...
        float *meanDataBuffer)
{
    ...
    {
        outBuffer[row * width + col] =
            scaleFactor[0] * ((float) inBuffer[row * width + col] -
            meanDataBuffer[(row * width) + col]);
    }
}
