...
bool
InferPreprocessor::setScaleOffsets(const std::vector<float>& scale, const std::vector<float>& offsets)
{
    if ((!offsets.empty() && m_NetworkInfo.channels != (uint32_t)offsets.size()) 
    ||  (!scale.empty() && m_NetworkInfo.channels != (uint32_t)scale.size()))
    {
        return false;
    }

    if (!offsets.empty() && !scale.empty())
    {
        m_ChannelMeans.assign(
            offsets.begin(), offsets.begin() + m_NetworkInfo.channels);
        m_Scale.assign(
            scale.begin(), scale.begin() + m_NetworkInfo.channels);
    }
    return true;
}
...
{
    if (!m_MeanFile.empty() || m_ChannelMeans.size() > 0 || m_Scale.size() > 0)
    {
        ...
        m_ScaleDataBuffer = std::make_unique<CudaDeviceBuffer>(
               (size_t) m_NetworkInfo.width * m_NetworkInfo.height * m_NetworkInfo.channels *
                sizeof(float));
        ...
        if (!m_ScaleDataBuffer || !m_ScaleDataBuffer->ptr())
        {
            printError("Failed to allocate cuda buffer for scale image");
            return NVDSINFER_CUDA_ERROR;
        }
    }
    ...
    else if (m_ChannelMeans.size() > 0)
    {
             ...
             j++)
        {
         ...
         std::vector<float> scaleData(m_NetworkInfo.channels *
                                    m_NetworkInfo.width * m_NetworkInfo.height);
        for (size_t j = 0; j < m_NetworkInfo.width * m_NetworkInfo.height;
             j++)
        {
            for (size_t i = 0; i < m_NetworkInfo.channels; i++)
            {
                scaleData[j * m_NetworkInfo.channels + i] = m_Scale[i];
            }
        }
        cudaError_t cudaReturn =
        ...
        cudaError_t cudaReturn1 =
            cudaMemcpy(m_ScaleDataBuffer->ptr(), scaleData.data(),
                scaleData.size() * sizeof(float), cudaMemcpyHostToDevice);
    ...
    for (unsigned int i = 0; i < batchSize; i++)
    {
        ...
        if (convertFcn) {
            ...
            convertFcn(outPtr, (unsigned char*)batchInput.inputFrames[i],
                ...
                m_ScaleDataBuffer.get() ? m_ScaleDataBuffer->ptr<float>() : nullptr,
            ...
            convertFcnFloat(outPtr, (float *)batchInput.inputFrames[i],
                m_NetworkInfo.width, m_NetworkInfo.height, batchInput.inputPitch,
                m_Scale.data(), m_MeanDataBuffer.get() ? m_MeanDataBuffer->ptr<float>() : nullptr,
                *m_PreProcessStream);
        }
    }
...
NvDsInferContextImpl::preparePreprocess(const NvDsInferContextInitParams& initParams)
{
    ...
    std::vector<float> scales(
            initParams.scales, initParams.scales + initParams.numScales);
    if (!processor->setScaleOffsets(
                scales, offsets)) {
        printError("Preprocessor set scale and offsets failed.");
        return NVDSINFER_CONFIG_FAILED;
